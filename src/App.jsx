...
import { ACTION_LABELS,advanceDate,avColor,DAYS,DEAL_PRIORITIES,DEAL_TYPES,DURATIONS,EMAIL_PROVIDERS,EMPLOYEE_ROLES,ETYPES,EXPENSE_CATEGORIES,EXPENSE_CATEGORY_COLORS,fmt$,fmtDate,fmtHours,fmtInvNum,fmtTime,FS_STAGES,ICP_LEVELS,INDUSTRIES,INV_COLORS,INVOICE_STATUSES,isDueToday,isFieldService,isLostStage,isOpenStage,isOverdueDate,isWonStage,LEAD_STATUSES,LIFECYCLE_STAGES,parseLocalDate,PLATFORMS,PRIORITIES,QUOTE_STATUSES,quoteStatusColor,quoteStatusLabel,RECURRING_FREQUENCIES,scoreColor,SERVICE_TYPES,SOURCES,STAGE_MIGRATION_MAP,stageColor,STAGES,stagesFor,TRIGGER_LABELS,uid,WEBHOOK_EVENTS } from "./lib/domain.js";
import { DEMO,DEMO_CALENDAR_EVENTS,DEMO_EMAIL_MESSAGES,DEMO_FULL } from "./lib/demoData.js";
import { calcDealHealth, calcLeadScore, DealHealthChip } from "./lib/scoring.jsx";
import { dateRangeBounds,exportReportCSV,exportReportPDF,inDateRange,REPORT_DATE_RANGES,REPORT_FIELDS,REPORT_TEMPLATES,REPORT_TYPE_LABELS,runReportRows } from "./lib/reports.js";
import { Avatar,F,Field,I,Ic,Modal,NoteRow,PageHeader,S,ScoreBadge,StatCard } from "./ui/primitives.jsx";
...
function ContactGmailTab({contactId,demoMode}){
  ...
  useEffect(()=>{
    if(demoMode){setMsgs(DEMO_EMAIL_MESSAGES.filter(m=>m.contact_id===contactId));return;}
    let cancelled=false;
  ...
  if(!msgs.length)return <EmptyState icon={I.inbox} title="No synced email yet" message={demoMode?"No email logged for this contact in the demo.":"Messages appear here once this entity's Google account is connected (Settings → Connected accounts) and a sync has run. Only mail involving CRM contacts is stored."}/>;
  return(
    <div>
      {msgs.map(m=>(
...
function GoogleCalendarView({account,entities,demoMode,contacts,companies=[],deals=[],notes=[],tasks=[],invoices=[],setSelContact,setView,showToast,briefingCtx}){
  ...
  useEffect(()=>{
    ...
    if(demoMode){
      // In-memory demo events (zero Supabase); filtered to the visible window.
      setEvents(DEMO_CALENDAR_EVENTS.filter(e=>{const s=new Date(e.start_at).getTime();return s>=from.getTime()&&s<=to.getTime();}));
      return;
    }
...
export default function App({session,onLogout,demoMode=false,account=null,accounts=[],onSwitchAccount}={}){
  ...
  const [signatures,setSignatures]=useState(D.signatures||[]);
  const [customReports,setCustomReports]=useState(D.customReports||[]);
  // Field Service
